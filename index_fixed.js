...
const SUPABASE_UPLOAD_BUCKET=process.env.SUPABASE_UPLOAD_BUCKET||"hillsbyte-uploads";
...
async function uploadProofToSupabase(file){
  ...
  try{
    await fetch(`${SUPABASE_URL}/storage/v1/bucket`,{
      ...
      body:JSON.stringify({id:SUPABASE_UPLOAD_BUCKET,name:SUPABASE_UPLOAD_BUCKET,public:true})
    }).catch(()=>{});
    const objectName=`proofs/${Date.now()}-${crypto.randomBytes(8).toString("hex")}${path.extname(file.originalname||"").toLowerCase()}`;
    const body=fs.readFileSync(file.path);
    const r=await fetch(`${SUPABASE_URL}/storage/v1/object/${encodeURIComponent(SUPABASE_UPLOAD_BUCKET)}/${objectName}`,{
      method:"POST",
      headers:{"Authorization":`Bearer ${SUPABASE_SERVICE_ROLE_KEY}`,"apikey":SUPABASE_SERVICE_ROLE_KEY,"Content-Type":file.mimetype,"x-upsert":"true"},
      body
    });
    if(!r.ok) return null;
    return `${SUPABASE_URL}/storage/v1/object/public/${encodeURIComponent(SUPABASE_UPLOAD_BUCKET)}/${objectName}`;
  }catch(e){
    console.error("Proof storage upload failed:",e.message);
    return null;
  }
}
...
async function restoreLocalUploads(){
  ...
  try{
    const r=await fetch(`${SUPABASE_URL}/storage/v1/object/list/${encodeURIComponent(SUPABASE_UPLOAD_BUCKET)}`,{
      method:"POST",
      headers:{"Authorization":`Bearer ${SUPABASE_SERVICE_ROLE_KEY}`,"apikey":SUPABASE_SERVICE_ROLE_KEY,"Content-Type":"application/json"},
      body:JSON.stringify({prefix:"uploads/",limit:1000,recursive:true})
    });
    if(!r.ok)return;
    const objects=await r.json();
    for(const obj of Array.isArray(objects)?objects:[]){
      if(!obj.name||!obj.name.startsWith("uploads/"))continue;
      const filename=path.basename(obj.name);
      const target=path.join(ROOT,"uploads",filename);
      if(fs.existsSync(target))continue;
      const file=await fetch(`${SUPABASE_URL}/storage/v1/object/${encodeURIComponent(SUPABASE_UPLOAD_BUCKET)}/${obj.name}`,{
        headers:{"Authorization":`Bearer ${SUPABASE_SERVICE_ROLE_KEY}`,"apikey":SUPABASE_SERVICE_ROLE_KEY}
      });
      if(file.ok)fs.writeFileSync(target,Buffer.from(await file.arrayBuffer()));
    }
  }catch(e){console.error("Upload restore skipped:",e.message)}
}
...
async function syncLocalUploads(){
  ...
  try{
    ...
    for(const filename of fs.readdirSync(dir)){
      ...
      await fetch(`${SUPABASE_URL}/storage/v1/object/${encodeURIComponent(SUPABASE_UPLOAD_BUCKET)}/${objectName}`,{
        method:"POST",
  ...
  }catch(e){console.error("Upload backup failed:",e.message)}
}
...
app.get("/api/admin/users",admin,(req,res)=>{
 const users=db.prepare("SELECT * FROM users ORDER BY id DESC LIMIT 500").all().map(u=>({
   ...u,
   completed_count:Number(db.prepare("SELECT count(*) c FROM submissions WHERE user_id=? AND status='approved'").get(u.id).c||0),
   pending_count:Number(db.prepare("SELECT count(*) c FROM submissions WHERE user_id=? AND status='pending'").get(u.id).c||0),
   rejected_count:Number(db.prepare("SELECT count(*) c FROM submissions WHERE user_id=? AND status='rejected'").get(u.id).c||0),
   watched_ads:Number(db.prepare("SELECT count(*) c FROM activity WHERE user_id=? AND kind='ad'").get(u.id).c||0),
   referral_count:Number(db.prepare("SELECT count(*) c FROM users WHERE referred_by=?").get(u.referral_code).c||0)
 }));
 res.json({users});
});
app.get("/api/admin/users/:id",admin,(req,res)=>{
 try{
   ...
   const chart=db.prepare(`WITH days AS (
      SELECT substr(created_at,1,10) day FROM activity WHERE user_id=? AND created_at>=datetime('now','-29 days')
      UNION SELECT substr(created_at,1,10) FROM users WHERE referred_by=? AND created_at>=datetime('now','-29 days')
    )
    SELECT d.day,
      COALESCE((SELECT COUNT(*) FROM activity WHERE user_id=? AND kind='earning' AND substr(created_at,1,10)=d.day),0) completed,
      COALESCE((SELECT COUNT(*) FROM activity WHERE user_id=? AND kind='ad' AND substr(created_at,1,10)=d.day),0) watched_ads,
      COALESCE((SELECT COUNT(*) FROM users WHERE referred_by=? AND substr(created_at,1,10)=d.day),0) referrals,
      COALESCE((SELECT SUM(amount) FROM activity WHERE user_id=? AND kind='earning' AND substr(created_at,1,10)=d.day),0) earnings
      FROM days d ORDER BY d.day`).all(u.id,u.referral_code,u.id,u.id,u.referral_code,u.id);
   const activity=db.prepare("SELECT a.*,t.title FROM activity a LEFT JOIN tasks t ON t.id=a.task_id WHERE a.user_id=? ORDER BY a.id DESC LIMIT 100").all(u.id);
   const referrals=db.prepare("SELECT id,telegram_id,username,first_name,status,created_at FROM users WHERE referred_by=? ORDER BY id DESC").all(u.referral_code);
   const counts={
     completed:Number(db.prepare("SELECT count(*) c FROM submissions WHERE user_id=? AND status='approved'").get(u.id).c||0),
     pending:Number(db.prepare("SELECT count(*) c FROM submissions WHERE user_id=? AND status='pending'").get(u.id).c||0),
     rejected:Number(db.prepare("SELECT count(*) c FROM submissions WHERE user_id=? AND status='rejected'").get(u.id).c||0),
     watched_ads:Number(db.prepare("SELECT count(*) c FROM activity WHERE user_id=? AND kind='ad'").get(u.id).c||0),
     referrals:referrals.length
   };
   res.json({user:{...u,...counts},chart,activity,referrals});
 }catch(e){res.status(400).json({error:e.message})}
});
...
app.delete("/api/admin/submissions",admin,(req,res)=>{
 try{
   const ids=Array.isArray(req.body?.ids)?req.body.ids.map(Number).filter(Number.isInteger):[];
   if(!ids.length)throw new Error("Select at least one proof.");
   const placeholders=ids.map(()=>"?").join(",");
   const rows=db.prepare(`SELECT id,screenshots FROM submissions WHERE id IN (${placeholders}) AND status IN ('approved','rejected')`).all(...ids);
   const tx=db.transaction(()=>{
     // Keep the reviewed submission record so completion history and retry protection remain intact;
     // remove only its proof payload so old screenshots no longer occupy the admin proof queue.
     db.prepare(`UPDATE submissions SET screenshots='[]' WHERE id IN (${placeholders}) AND status IN ('approved','rejected')`).run(...ids);
   });
   tx();
   for(const row of rows){
     for(const src of parseScreenshots(row.screenshots)){
       if(src.startsWith("/uploads/")){
         try{fs.unlinkSync(path.join(UPLOAD_DIR,path.basename(src)))}catch{}
       }
     }
   }
...
app.get("/api/admin/submissions",admin,(req,res)=>{
  const rows=db.prepare(`SELECT s.*,u.username,u.first_name,u.telegram_id,t.title,t.reward
    ...
    WHERE s.screenshots IS NOT NULL AND s.screenshots <> '[]'
...
app.post("/api/assistant",async(req,res)=>{
  try{
    ...
    if(process.env.OPENAI_API_KEY){
      const r=await fetch("https://api.openai.com/v1/responses",{
        ...
        body:JSON.stringify({
          ...
          input:[
       {role:"system",content:`You are HillsByte AI, a friendly general-purpose chat assistant inside a Telegram Mini App. You can discuss normal everyday topics too: study, technology, coding concepts, writing, ideas, productivity, money basics, and casual conversation. For HillsByte account questions, use the user's supplied stats and explain app navigation accurately. Never claim to have performed an action you did not perform, never promise earnings, and never approve payments. User account context: first name=${u.first_name||"Member"}, balance=$${Number(stats.balance||0).toFixed(2)}, completed tasks=${stats.completed_count}, pending proofs=${stats.pending_tasks}, referrals=${stats.referral_count}, HillsCoin=${stats.hillscoin}.`},
       ...history,
    ...
    let answer="I can chat about HillsByte or general topics. Tell me what you need help with, and I’ll keep the conversation going.";
