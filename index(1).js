...
function storageObjectUrl(objectPath){ return `${SUPABASE_URL}/storage/v1/object/${encodeURIComponent(SUPABASE_BUCKET)}/${objectPath.split("/").map(encodeURIComponent).join("/")}`; }
async function uploadFileToSupabase(filePath,objectPath,contentType){ if(!SUPABASE_URL||!SUPABASE_SERVICE_ROLE_KEY)return false; const body=fs.readFileSync(filePath); const r=await fetch(storageObjectUrl(objectPath),{method:"POST",headers:{...supabaseHeaders(),"Content-Type":contentType||"application/octet-stream","x-upsert":"true"},body}); if(!r.ok){const text=await r.text().catch(()=>"");throw new Error(`Supabase upload failed (${r.status}) ${text.slice(0,250)}`);} return true; }
function publicStorageObjectUrl(objectPath){ return `${SUPABASE_URL}/storage/v1/object/public/${encodeURIComponent(SUPABASE_BUCKET)}/${objectPath.split("/").map(encodeURIComponent).join("/")}`; }
function storagePathFromUrl(value){ const s=String(value||""); const marker=`/storage/v1/object/public/${SUPABASE_BUCKET}/`; const i=s.indexOf(marker); return i>=0?decodeURIComponent(s.slice(i+marker.length)):""; }
async function removeStorageObject(objectPath){ if(!SUPABASE_URL||!SUPABASE_SERVICE_ROLE_KEY||!objectPath)return false; const r=await fetch(`${SUPABASE_URL}/storage/v1/object/${encodeURIComponent(SUPABASE_BUCKET)}/${objectPath.split("/").map(encodeURIComponent).join("/")}`,{method:"DELETE",headers:supabaseHeaders()}); if(!r.ok&&r.status!==404)throw new Error(`Supabase delete failed (${r.status})`); return true; }
async function persistUploadedFiles(files,folder){ if(!files?.length)return []; if(!SUPABASE_URL||!SUPABASE_SERVICE_ROLE_KEY)return files.map(f=>`/uploads/${path.basename(f.path)}`); await ensureSupabaseBucket(); const urls=[]; for(const f of files){ const objectPath=`${folder}/${Date.now()}-${crypto.randomBytes(6).toString("hex")}${path.extname(f.originalname||f.path||"").toLowerCase()}`; await uploadFileToSupabase(f.path,objectPath,f.mimetype); urls.push(publicStorageObjectUrl(objectPath)); fs.rmSync(f.path,{force:true}); } return urls; }
...
async function ensureSupabaseBucket(){
  ...
  try{
    const check=await fetch(`${SUPABASE_URL}/storage/v1/bucket/${encodeURIComponent(SUPABASE_BUCKET)}`,{
    ...
    console.error("Supabase Storage setup failed:",e.message);
    return false;
  }
}
...
app.post("/api/submissions",upload.array("screenshots",3),async(req,res)=>{
 try{
  ...
  const shots=await persistUploadedFiles(req.files||[],"proofs");
...
app.post("/api/admin/submissions/bulk-delete",admin,async(req,res)=>{ try{ const ids=Array.isArray(req.body?.ids)?req.body.ids.map(Number).filter(Number.isInteger):[]; if(!ids.length)throw new Error("Select at least one reviewed proof."); const ph=ids.map(()=>"?").join(","); const rows=db.prepare(`SELECT id,screenshots FROM submissions WHERE id IN (${ph}) AND status IN ('approved','rejected')`).all(...ids); db.prepare(`DELETE FROM submissions WHERE id IN (${ph}) AND status IN ('approved','rejected')`).run(...ids); for(const row of rows)for(const shot of parseScreenshots(row.screenshots)){const sp=storagePathFromUrl(shot);if(sp)await removeStorageObject(sp).catch(e=>console.error("Proof storage delete:",e.message));} res.json({ok:true,deleted:rows.length}); }catch(e){res.status(400).json({error:e.message})} });
app.get("/api/admin/submissions/reviewed",admin,(req,res)=>{ try{ const sort=String(req.query.sort||"newest"); const orders={newest:"s.id DESC",oldest:"s.id ASC",approved:"CASE WHEN s.status='approved' THEN 0 ELSE 1 END,s.id DESC",rejected:"CASE WHEN s.status='rejected' THEN 0 ELSE 1 END,s.id DESC",reward_high:"t.reward DESC,s.id DESC",reward_low:"t.reward ASC,s.id DESC"}; const rows=db.prepare(`SELECT s.*,u.username,u.first_name,u.telegram_id,t.title,t.reward FROM submissions s JOIN users u ON u.id=s.user_id JOIN tasks t ON t.id=s.task_id WHERE s.status IN ('approved','rejected') ORDER BY ${orders[sort]||orders.newest} LIMIT 500`).all(); res.json({submissions:rows.map(s=>({...s,screenshots:parseScreenshots(s.screenshots)}))}); }catch(e){res.status(400).json({error:e.message})} });

// Delete a single reviewed proof (approved or rejected only) and its stored screenshots.
app.delete("/api/admin/submissions/:id",admin,async(req,res)=>{
  try{
    const s=db.prepare("SELECT * FROM submissions WHERE id=?").get(req.params.id);
    if(!s) return res.status(404).json({error:"Not found"});
    if(!["approved","rejected"].includes(s.status)) throw new Error("Only reviewed proofs can be deleted. Pending proofs must be approved or rejected first.");
    db.prepare("DELETE FROM submissions WHERE id=?").run(s.id);
    for(const shot of parseScreenshots(s.screenshots)){
      const sp=storagePathFromUrl(shot);
      if(sp) await removeStorageObject(sp).catch(e=>console.error("Proof storage delete:",e.message));
    }
    res.json({ok:true});
  }catch(e){res.status(400).json({error:e.message})}
});

// Full activity timeline for one user: earnings, submissions, withdrawals,
// notifications, referrals and balance info. Use this when an admin taps a user's profile.
app.get("/api/admin/users/:id/details",admin,(req,res)=>{
  try{
    const u=db.prepare("SELECT * FROM users WHERE id=?").get(req.params.id);
    if(!u) return res.status(404).json({error:"User not found"});

    const activity=db.prepare(`
      SELECT a.id,a.kind,a.amount,a.task_id,a.created_at,t.title AS task_title
      FROM activity a LEFT JOIN tasks t ON t.id=a.task_id
      WHERE a.user_id=? ORDER BY a.id DESC LIMIT 200
    `).all(u.id);

    const submissions=db.prepare(`
      SELECT s.id,s.status,s.note,s.created_at,s.screenshots,t.title,t.reward
      FROM submissions s JOIN tasks t ON t.id=s.task_id
      WHERE s.user_id=? ORDER BY s.id DESC LIMIT 100
    `).all(u.id).map(s=>({...s,screenshots:parseScreenshots(s.screenshots)}));

    const withdrawals=db.prepare("SELECT * FROM withdrawals WHERE user_id=? ORDER BY id DESC LIMIT 100").all(u.id);
    const notifications=db.prepare("SELECT * FROM notifications WHERE user_id=? ORDER BY id DESC LIMIT 100").all(u.id);
    const referrals=db.prepare("SELECT id,telegram_id,username,first_name,status,created_at FROM users WHERE referred_by=? ORDER BY id DESC LIMIT 50").all(u.referral_code);
    const promoRedemptions=db.prepare(`
      SELECT p.code,p.amount,p.coin_amount,r.created_at
      FROM promo_redemptions r JOIN promos p ON p.id=r.promo_id
