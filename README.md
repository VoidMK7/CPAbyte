...
The app restores the SQLite database from Supabase Storage only when the local database is missing, then syncs database writes back to Supabase. Uploaded proof screenshots are stored in the configured Supabase Storage bucket and served back through short-lived signed URLs, so Render restarts do not make new screenshots disappear. Keep `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, and `SUPABASE_BUCKET` configured in Render.
