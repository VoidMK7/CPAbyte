...
.adminToolbar{display:flex;gap:8px;flex-wrap:wrap;align-items:center}
.adminToolbar select,.adminToolbar button{min-height:38px}
.adminSelectionBar{padding:8px 0;font-size:12px;opacity:.7}
.adminSubmissionCard.selected{outline:1px solid rgba(34,197,94,.55)}
.submissionIdentity{display:flex;align-items:center;gap:10px}
.proofImageButton{background:none;border:0;padding:0;cursor:pointer;color:inherit;text-align:left}
.adminProfileModal{max-height:88vh;overflow:auto}
.adminProfileStats{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:12px 0}
.adminProfileStats>div{padding:12px;border-radius:14px;background:rgba(255,255,255,.05);display:flex;flex-direction:column;gap:4px}
.adminProfileStats b{font-size:18px}.adminProfileStats span,.adminReferralRow small{font-size:11px;opacity:.65}
.adminProfileChart{margin:14px 0;padding:12px;border-radius:14px;background:rgba(255,255,255,.04)}
.activityBars{height:130px;display:flex;align-items:end;gap:4px;overflow-x:auto}
.activityBar{min-width:18px;height:100%;display:flex;flex-direction:column;justify-content:end;align-items:center;gap:3px}
.activityBar i{display:block;width:12px;min-height:6px;border-radius:5px 5px 2px 2px;background:currentColor;opacity:.75}
.activityBar small{font-size:8px;opacity:.5}
.adminReferralRow{display:flex;justify-content:space-between;gap:12px;padding:9px 0;border-bottom:1px solid rgba(255,255,255,.06)}
.adminProfileDetails{margin-top:16px}
@media(max-width:600px){.adminProfileStats{grid-template-columns:repeat(2,1fr)}.adminToolbar{width:100%}.adminToolbar>*{flex:1}}
`}</style>      {tab==="home"&&<Home me={me} tasks={tasks} onStart={startTask} setTab={setTab}/>}
...
function TaskRunner({task,attempt,onDone,setToast}){
  ...
  return <section>
    ...
    <div className="runner">
      ...
      {!gate?<><div className="instructions"><h3>How to complete</h3><p className="multiline">{cleanText(task.description)}</p>{task.url&&<a href={task.url} target="_blank" rel="noreferrer">Open task link ↗</a>}</div>
...
function AssistantPage({setToast}){
  ...
  const ask=async()=>{
    ...
    try{const d=await api("/api/assistant",{method:"POST",body:JSON.stringify({telegramId:String(tgUser().id),question:q,history:messages})});setMessages(m=>[...m,{role:"assistant",text:d.answer}])}catch(e){setToast(e.message)}finally{setBusy(false)}
  };
  return <section className="assistantPage">
    ...
    <div className="quickQuestions">{["Help me plan my day","Explain something I’m learning","How do I complete a task?","Let’s just chat"].map(q=><button key={q} onClick={()=>setQuestion(q)}>{q}</button>)}</div>
  </section>;
}
...
function AdminPage({setToast}){
  ...
  const [proofSort,setProofSort]=useState("newest");
  ...
  const deleteReviewedProofs=async(ids=selectedProofs)=>{
    const clean=ids.map(Number).filter(id=>submissions.some(s=>s.id===id&&s.status!=="pending"));
    if(!clean.length)return setToast("Select reviewed proofs first.");
    if(!confirm(`Delete ${clean.length} reviewed proof${clean.length===1?"":"s"}? This removes them from proof history.`))return;
    try{
      await api("/api/admin/submissions",{method:"DELETE",body:JSON.stringify({ids:clean})});
      setSelectedProofs([]);
      setToast("Reviewed proofs deleted.");
      await load();
    }catch(e){setToast(e.message)}
  };
  ...
  const openUserProfile=async(id)=>{
    try{
      ...
      setUserProfile(await api(`/api/admin/users/${id}`));
    }catch(e){setToast(e.message)}
...
{tab==="broadcast"&&<div className="adminContent">
  <div className="adminPanel">
    ...
    {tab==="submissions"&&<div className="adminContent">
      <div className="adminPanel">
        <div className="adminPanelHeader">
          <div><h3>Proof review</h3><p>Review screenshots without leaving the Mini App.</p></div>
          <div className="adminToolbar">
            <select value={proofSort} onChange={e=>setProofSort(e.target.value)}>
              <option value="newest">Newest first</option><option value="oldest">Oldest first</option>
              <option value="pending">Pending first</option><option value="reviewed">Reviewed first</option>
            </select>
            <button onClick={()=>setSelectedProofs(submissions.filter(s=>s.status!=="pending").map(s=>s.id))}>Select reviewed</button>
            <button onClick={()=>setSelectedProofs(submissions.map(s=>s.id))}>Select all</button>
            <button className="dangerBtn" disabled={!selectedProofs.length} onClick={()=>deleteReviewedProofs()}>Delete selected</button>
          </div>
        </div>
        <div className="adminSelectionBar">{selectedProofs.length} selected · {submissions.filter(s=>s.status!=="pending").length} reviewed</div>
        <div className="adminSubmissionList">
          {submissions.length===0&&<div className="adminEmpty">No submissions found.</div>}
          {[...submissions].sort((a,b)=>{
            if(proofSort==="oldest")return a.id-b.id;
            if(proofSort==="pending")return (a.status==="pending"?0:1)-(b.status==="pending"?0:1)||b.id-a.id;
            if(proofSort==="reviewed")return (a.status==="pending"?1:0)-(b.status==="pending"?1:0)||b.id-a.id;
            return b.id-a.id;
          }).map(s=><article className={`adminSubmissionCard ${selectedProofs.includes(s.id)?"selected":""}`} key={s.id}>
            <div className="submissionIdentity">
              <input type="checkbox" checked={selectedProofs.includes(s.id)} onChange={e=>setSelectedProofs(x=>e.target.checked?[...x,s.id]:x.filter(id=>id!==s.id))}/>
              <div className="adminUserAvatar">{(s.first_name||s.username||"U").charAt(0).toUpperCase()}</div>
              <div><b>{cleanText(s.title||"Task submission")}</b><small>{s.first_name||"Telegram user"} {s.username?`· @${s.username}`:""}</small><small>Reward ${Number(s.reward||0).toFixed(2)} · {s.status}</small></div>
            </div>
            <div className="proofThumbs">
              {(s.screenshots||[]).slice(0,3).map((src,i)=><button type="button" key={src} onClick={()=>setSelectedSubmission({...s,focus:i})}><img src={src} alt={`Proof ${i+1}`}/></button>)}
      ...
      <div className="adminPanel">
        ...
        <div className="adminUserList">
          ...
          {users.map(u=><article className="adminUserCard" key={u.id}>
            ...
            <div className="adminUserInfo"><b>{u.first_name||"Telegram member"}</b><small>{u.username?`@${u.username}`:"No username"} · ID {u.telegram_id}</small><small>${Number(u.balance||0).toFixed(2)} balance · {u.status}</small><small>{u.completed_count||0} completed · {u.watched_ads||0} ads · {u.referral_count||0} referrals</small></div>
            <div className="adminUserActions"><button onClick={()=>openUserProfile(u.id)}>View profile</button><button onClick={()=>messageUser(u.id)}>Message</button><button onClick={()=>act(u.id,u.status==="banned"?"unban":"ban")}>{u.status==="banned"?"Unban":"Ban"}</button></div>
          </article>)}
        </div>
      </div>
    </div>}
...
{userProfile&&<div className="proofModal" onClick={()=>setUserProfile(null)}>
      <div className="proofModalInner adminProfileModal" onClick={e=>e.stopPropagation()}>
        <div className="proofModalHeader"><div><h3>{userProfile.user.first_name||"Telegram member"}</h3><p>{userProfile.user.username?`@${userProfile.user.username} · `:""}Telegram ID {userProfile.user.telegram_id}</p></div><button onClick={()=>setUserProfile(null)}>Close</button></div>
        <div className="adminProfileStats">
          <div><b>${Number(userProfile.user.balance||0).toFixed(2)}</b><span>Balance</span></div>
          <div><b>{userProfile.user.completed}</b><span>Completed</span></div>
          <div><b>{userProfile.user.watched_ads}</b><span>Watched ads</span></div>
          <div><b>{userProfile.user.referrals}</b><span>Referrals</span></div>
        </div>
        <div className="adminProfileChart">
          <h4>30-day activity · tasks / ads / referrals</h4>
          {userProfile.chart?.length?<div className="activityBars">{userProfile.chart.map(x=><div className="activityBar" key={x.day}><i style={{height:`${Math.max(6,Math.min(100,((Number(x.completed)+Number(x.watched_ads)+Number(x.referrals||0))*12))}%`}}/><small>{x.day.slice(5)}</small></div>)}</div>:<p className="muted">No activity recorded yet.</p>}
        </div>
        <div className="adminProfileDetails"><h4>Referral activity</h4>{userProfile.referrals?.length?userProfile.referrals.map(r=><div className="adminReferralRow" key={r.id}><span>{r.first_name||r.username||r.telegram_id}</span><b>{r.status==="active"?"Active":"Not active"}</b></div>):<p className="muted">No referrals yet.</p>}</div>
        <div className="adminProfileDetails"><h4>Recent activity</h4>{userProfile.activity?.slice(0,20).map(a=><div className="adminReferralRow" key={a.id}><span>{a.kind==="earning"?"Task completed":a.kind==="ad"?"Ad watched":a.kind}</span><small>{new Date(a.created_at).toLocaleString()}</small></div>)}</div>
      </div>
    </div>}
    {selectedSubmission&&<div className="proofModal" onClick={()=>setSelectedSubmission(null)}>
      <div className="proofModalInner" onClick={e=>e.stopPropagation()}>
        ...
        {selectedSubmission.note&&<div className="proofNote"><b>User note</b><p className="multiline">{cleanText(selectedSubmission.note)}</p></div>}
        <div className="proofGallery">
          {(selectedSubmission.screenshots||[]).map((src,i)=><button type="button" className="proofImageButton" key={src} onClick={()=>setSelectedSubmission({...selectedSubmission,focus:i})}><img src={src} alt={`Proof ${i+1}`}/><span>View proof {i+1}</span></button>)}
        </div>
