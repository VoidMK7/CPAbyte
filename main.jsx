        ...
        <div className="adminSubmissionList">
          ...
          {filteredProofs.map(s=><article className="adminSubmissionCard" key={s.id}>
            ...
            <div className="proofThumbs">
              {(s.screenshots||[]).slice(0,3).map((src,i)=><button key={src} onClick={()=>{setSelectedSubmission(s);setSelectedProofIndex(i);setProofZoom(false)}}><img src={src} alt={`Proof ${i+1}`}/></button>)}
...
{selectedSubmission&&<div className="proofModal" onClick={()=>setSelectedSubmission(null)}>
      <div className="proofModalInner" onClick={e=>e.stopPropagation()}>
        ...
        <div className="proofViewer">
          <div className={`proofFullImage ${proofZoom?"zoomed":""}`}><img src={(selectedSubmission.screenshots||[])[selectedProofIndex]} alt={`Proof ${selectedProofIndex+1}`}/></div>
          ...
          <div className="proofThumbs large">{(selectedSubmission.screenshots||[]).map((src,i)=><button className={i===selectedProofIndex?"selected":""} key={src} onClick={()=>{setSelectedProofIndex(i);setProofZoom(false)}}><img src={src} alt={`Proof ${i+1}`}/></button>)}</div>
        </div>
