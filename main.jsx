...
function App(){
  ...
  const [admin,setAdmin]=useState(false),[drawerOpen,setDrawerOpen]=useState(false),[reviewer,setReviewer]=useState(false),[moderator,setModerator]=useState(false),[reviewerApplication,setReviewerApplication]=useState(null),[unread,setUnread]=useState(0),[broadcasts,setBroadcasts]=useState([]),[events,setEvents]=useState([]);
  ...
  const load=async()=>{
    try{
      ...
      setMe({...d.user,min_withdrawal:d.minWithdrawal||1});setTasks(d.tasks||[]);setAdmin(!!d.admin);setReviewer(!!d.reviewer);setModerator(!!d.moderator);setReviewerApplication(d.reviewerApplication||null);setBroadcasts(d.broadcasts||[]);setEvents(d.events||[]);
      try{
  ...
  if(!me) return <div className="splash">
    <div className="orbitalScene" aria-hidden="true"><div className="orbit orbitOne"><i/></div><div className="orbit orbitTwo"><i/></div><div className="orbit orbitThree"><i/></div><div className="orbitalCore">H</div></div>
    <h1>HILLSBYTE</h1>
    <p>Complete tasks. Earn rewards. Grow.</p>
    <button className="primary" onClick={()=>load()}>Open HillsByte</button>
  </div>;

  const go=(next)=>{setTab(next);setDrawerOpen(false)};
  const menuItems=[
    ["home","Dashboard"],["tasks","Earn"],["wallet","Wallet"],["referrals","Referrals"],["community","Community"],["profile","Profile"],
    ["ads","Ads"],["assistant","AI Assistant"],...(reviewer?[["review","Review"]]:[]),...(admin?[["admin","Admin Panel"]]:[])
  ];

  return <div className="appShell">
    <header className="topbar">
      <button className="menuButton" aria-label="Open menu" onClick={()=>setDrawerOpen(true)}><Menu size={23}/></button>
      <div className="brandBlock"><div className="brand">HILLS<span>BYTE</span></div><small>Earn. Complete. Grow.</small></div>
      <button className="topProfile" onClick={()=>go("profile")}>
    ...
    {drawerOpen&&<><button className="drawerBackdrop" aria-label="Close menu" onClick={()=>setDrawerOpen(false)}/><aside className="sideDrawer">
      <div className="drawerHead"><div><b>HILLS<span>BYTE</span></b><small>Menu</small></div><button onClick={()=>setDrawerOpen(false)}><X/></button></div>
      <nav>{menuItems.map(([id,label],i)=><button key={`${id}-${i}`} className={tab===id?"active":""} onClick={()=>go(id)}>{label}</button>)}</nav>
    </aside></>}

    <main>
      {tab==="home"&&<Home me={me} broadcasts={broadcasts} events={events} setTab={setTab}/>}
      {tab==="tasks"&&<TasksPage tasks={tasks} onStart={startTask}/>} 
      {tab==="community"&&<CommunityPage me={me} setToast={setToast}/>} 
...
function Home({me,broadcasts,events,setTab}){
  return <section className="homePage">
    ...
    <div className="dashboardCta">
      <div><span className="eyebrow">READY TO EARN?</span><h2>Start earning</h2><p>View available tasks and choose one to get started.</p></div>
      <button className="primary" onClick={()=>setTab("tasks")}>View available tasks <ArrowUpRight size={15}/></button>
    </div>

    <div className="dashboardPost profilePanel">
      <div><span className="eyebrow">COMMUNITY</span><h3>Have something to share?</h3><p>Post a message for the HillsByte community.</p></div>
      <button className="secondaryBtn" onClick={()=>setTab("community")}>Post to community <Send size={15}/></button>
    </div>

    <div className="dashboardGrid">
      <div className="profilePanel"><div className="panelTitle"><div><h3>Events</h3><p>Upcoming HillsByte events.</p></div><Megaphone/></div><div className="eventList">
        ...
        {!events.length&&<div className="profileEmpty">No upcoming events.</div>}
      </div></div>
      <div className="profilePanel"><div className="panelTitle"><div><h3>Announcements</h3><p>Latest official HillsByte updates.</p></div><Bell/></div><div className="eventList">
        {broadcasts.map(b=><div className="eventCard" key={`broadcast-${b.id}`}><div><b>HillsByte announcement</b><p>{b.message||"New update"}</p><small>{new Date(b.created_at+"Z").toLocaleString()}</small></div></div>)}
        {!broadcasts.length&&<div className="profileEmpty">No announcements yet.</div>}
      </div></div>
    </div>

    <div className="profilePanel dashboardAdCard"><div className="panelTitle"><div><h3>Ads & Sponsors</h3><p>Optional sponsored activities may provide extra earning opportunities.</p></div><Megaphone/></div><button className="secondaryBtn" onClick={()=>setTab("ads")}>Open Ads <ArrowUpRight size={15}/></button></div>
  </section>;
}

function TasksPage({tasks,onStart}){
  ...
  return <section className="tasksPage"><div className="sectionHead"><div><span className="eyebrow">EARN</span><h2>Available tasks</h2></div><span>{filtered.length} available</span></div><div className="taskSearch"><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search tasks..."/><span>{filtered.length}</span></div>{filtered.length===0?<div className="empty"><ListChecks size={28}/><b>No available tasks</b><span>Check back when new tasks are posted.</span></div>:<div className="taskList">{filtered.map(t=><TaskCard key={t.id} task={t} onStart={()=>onStart(t)}/>)}</div>}</section>;
}

function CommunityPage({me,setToast}){
  const [posts,setPosts]=useState([]),[message,setMessage]=useState(""),[busy,setBusy]=useState(false);
  const load=async()=>{try{const d=await api("/api/community/posts");setPosts(d.posts||[])}catch(e){setToast(e.message)}};
  useEffect(()=>{load()},[]);
  const post=async()=>{const text=message.trim();if(!text)return;try{setBusy(true);await api("/api/community/posts",{method:"POST",body:JSON.stringify({telegramId:String(tgUser().id),message:text})});setMessage("");setToast("Posted to the community.");load()}catch(e){setToast(e.message)}finally{setBusy(false)}};
  return <section className="communityPage"><div className="communityHero profilePanel"><div><span className="eyebrow">HILLSBYTE COMMUNITY</span><h2>Community</h2><p>Share updates, questions and helpful messages with other members.</p></div><Users/></div><div className="profilePanel communityComposer"><textarea rows="3" value={message} onChange={e=>setMessage(e.target.value)} placeholder="Write a message to the community..."/><button className="primary" disabled={busy} onClick={post}>{busy?"Posting…":"Post message"}</button></div><div className="communityFeed">{posts.map(p=><article className="profilePanel communityPost" key={p.id}><div className="communityPostHead"><div className="miniAvatar">{(p.first_name||"U")[0].toUpperCase()}</div><div><b>{p.first_name||"Member"}{p.username?` · @${p.username}`:""}</b><small>{new Date(p.created_at+"Z").toLocaleString()}</small></div></div><p>{p.message}</p></article>)}{!posts.length&&<div className="profileEmpty">No community posts yet.</div>}</div></section>;
}
...
function WalletPage({me,reload,setToast}){
  ...
  const minimum=Math.max(1,Number(me.min_withdrawal||1));
  const [method,setMethod]=useState("bank"),[amount,setAmount]=useState(Math.min(balance,Math.max(minimum,balance||minimum)).toFixed(2)),[form,setForm]=useState({accountName:"",accountNumber:"",bankName:"",network:"BEP20",address:"",memo:""});
  useEffect(()=>{setAmount(balance>=minimum?minimum.toFixed(2):"0.00")},[me.balance,minimum]);
  const chosen=Number(amount);
  const withdraw=async()=>{try{if(!Number.isFinite(chosen)||chosen<minimum)throw new Error(`Minimum withdrawal is $${minimum.toFixed(2)}.`);if(chosen>balance)throw new Error("You cannot withdraw more than your available balance.");await api("/api/withdrawals",{method:"POST",body:JSON.stringify({telegramId:String(tgUser().id),method,amount:chosen,...form})});setToast(`$${chosen.toFixed(2)} withdrawal request submitted.`);reload()}catch(e){setToast(e.message)}};
  return <section><div className="sectionHead"><div><h2>Wallet</h2><p>Manage your payout details</p></div><span className="livePill">Min ${minimum.toFixed(2)}</span></div>
    <div className="walletHero"><span>Available to withdraw</span><b>${balance.toFixed(2)}</b><small>Pending ${Number(me.pending_balance||0).toFixed(2)}</small></div>
    <div className="withdrawAmount"><label>Withdrawal amount</label><div className="amountInput"><span>$</span><input type="number" min={minimum} max={balance} step="0.01" value={amount} onChange={e=>setAmount(e.target.value)}/><button type="button" onClick={()=>setAmount(balance.toFixed(2))}>MAX</button></div><div className="amountQuick">{[minimum,balance/2,balance].filter((v,i,a)=>v>=minimum&&v>0&&a.indexOf(v)===i).map(v=><button key={v} type="button" onClick={()=>setAmount(v.toFixed(2))}>${v.toFixed(2)}</button>)}</div></div>
    ...
    <button className="primary" disabled={chosen<minimum||chosen>balance||balance<minimum} onClick={withdraw}>Request ${Number.isFinite(chosen)?chosen.toFixed(2):"0.00"} payout</button>
  </section>;
}
...
function AdsPage({setToast}){
  ...
  return <section className="adsPage">
    ...
    {ads?.enabled&&<button className="primary monetagButton" onClick={async()=>{try{if(typeof window.show_11725898!=="function")throw new Error("The ad provider has not loaded yet. Try again in a moment.");await window.show_11725898({type:"inApp",inAppSettings:{frequency:2,capping:0.1,interval:30,timeout:5,everyPage:false}})}catch(e){setToast(e.message)}}}>Watch sponsored ad</button>}
...
function AdminPage({setToast}){
  ...
  const [events,setAdminEvents]=useState([]);
  ...
  const load=async()=>{
    try{
      ...
      const results=await Promise.allSettled([
        ...
        api("/api/admin/submissions"),api("/api/admin/ads"),api("/api/admin/events"),api("/api/admin/reviewer-applications"),api("/api/admin/roles")
      ]);
      const [td,ud,wd,sd,ad,ev,apps,roleData]=results.map(r=>r.status==="fulfilled"?r.value:null);
      if(td) setTasks(td.tasks||[]);
      if(ud) setUsers(ud.users||[]);
      if(wd) setWithdrawals(wd.withdrawals||[]);
      if(sd){setSubmissions(sd.submissions||[]);setSelectedProofs(new Set());}
      if(ev) setAdminEvents(ev.events||[]);
      if(ad){
        setAds(ad.ads||null);
        if(ad.ads){
          setAdForm({
      ...
      if(apps) setApplications(apps.applications||[]);
      if(roleData) setRoles(roleData||{reviewers:[],moderators:[]});
      const failed=results.find(r=>r.status==="rejected");
      if(failed) setToast(failed.reason?.message||"One or more admin sections could not be loaded.");
    }catch(e){setToast(e.message)}
...
{tab==="broadcast"&&<div className="adminContent">
  <div className="adminPanel">
    <div className="adminPanelHeader">
      <div>
        <h3>📢 Announcements</h3>
        <p>Publish an official announcement to active HillsByte users.</p>
      </div>
    </div>
    ...
    {tab==="events"&&<div className="adminContent"><div className="adminPanel"><div className="adminPanelHeader"><div><h3>Events</h3><p>Create and manage events shown on the user dashboard.</p></div></div><div className="adminFormGrid"><input placeholder="Event title" value={eventForm.title} onChange={e=>setEventForm({...eventForm,title:e.target.value})}/><input type="datetime-local" value={eventForm.starts_at} onChange={e=>setEventForm({...eventForm,starts_at:e.target.value})}/><textarea className="wideField" placeholder="Event description" value={eventForm.description} onChange={e=>setEventForm({...eventForm,description:e.target.value})}/></div><button className="primary adminPublish" onClick={async()=>{try{if(!eventForm.title.trim()||!eventForm.starts_at)throw new Error("Event title and start time are required.");await api("/api/admin/events",{method:"POST",body:JSON.stringify(eventForm)});setEventForm({title:"",description:"",starts_at:""});setToast("Event created.");load()}catch(e){setToast(e.message)}}}>Create event</button><div className="adminList">{events.map(e=><div className="adminRow" key={e.id}><div><b>{e.title}</b><small>{e.description||"No description"} · {new Date(e.starts_at.replace(" ","T")+"Z").toLocaleString()}</small></div><button className="dangerBtn" onClick={async()=>{try{await api(`/api/admin/events/${e.id}`,{method:"DELETE"});setToast("Event deleted.");load()}catch(err){setToast(err.message)}}}>Delete</button></div>)}{!events.length&&<div className="adminEmpty">No events created.</div>}</div></div></div>}
