...
function AdminPage({setToast}){
  ...
  const openMemberProfile=async(user)=>{
    try{
      setMemberLoading(true);
      setMemberProfile({user});
      const d=await api(`/api/admin/users/${user.id}`);
      setMemberProfile(d);
    }catch(e){setToast(e.message);setMemberProfile(null)}
    finally{setMemberLoading(false)}
  };

  const reviewedSubmissions=submissions.filter(s=>["approved","rejected"].includes(s.status) && (s.screenshots||[]).length>0);
  const sortedSubmissions=useMemo(()=>{
    const list=[...submissions];
    if(proofSort==="oldest") list.sort((a,b)=>Number(a.id)-Number(b.id));
    else if(proofSort==="user") list.sort((a,b)=>String(a.first_name||a.username||"").localeCompare(String(b.first_name||b.username||"")));
    else if(proofSort==="reward") list.sort((a,b)=>Number(b.reward||0)-Number(a.reward||0));
    else list.sort((a,b)=>Number(b.id)-Number(a.id));
    return list;
  },[submissions,proofSort]);

  const toggleProofSelection=(id)=>{
    setSelectedProofIds(prev=>prev.includes(id)?prev.filter(x=>x!==id):[...prev,id]);
  };
  const selectAllReviewed=()=>{
    const ids=reviewedSubmissions.map(s=>s.id);
    setSelectedProofIds(prev=>prev.length===ids.length?[]:ids);
  };
  const deleteReviewedProofs=async()=>{
    const ids=selectedProofIds.length?selectedProofIds:reviewedSubmissions.map(s=>s.id);
    if(!ids.length){setToast("There are no reviewed proofs to delete.");return}
    if(!confirm(`Delete screenshots from ${ids.length} reviewed submission${ids.length===1?"":"s"}? Review history will be kept.`))return;
    try{
      const d=await api("/api/admin/submissions/reviewed-proofs",{method:"DELETE",body:JSON.stringify({ids})});
      setSelectedProofIds([]);
      setSelectedSubmission(null);
      setToast(`${d.deleted||0} reviewed proof${d.deleted===1?"":"s"} deleted. History was kept.`);
      await load();
    }catch(e){setToast(e.message)}
  };
...
{tab==="broadcast"&&<div className="adminContent">
  <div className="adminPanel">
    ...
    {tab==="submissions"&&<div className="adminContent">
      <div className="adminPanel">
        <div className="adminPanelHeader"><div><h3>Proof review</h3><p>Open screenshots inside the app. Reviewed-proof deletion keeps the task history.</p></div></div>
        <div className="proofToolbar">
          <button onClick={selectAllReviewed} disabled={!reviewedSubmissions.length}>{selectedProofIds.length===reviewedSubmissions.length&&reviewedSubmissions.length?"Clear selection":"Select all reviewed"}</button>
          <select value={proofSort} onChange={e=>setProofSort(e.target.value)}>
            <option value="newest">Sort: newest</option>
            <option value="oldest">Sort: oldest</option>
            <option value="user">Sort: member</option>
            <option value="reward">Sort: reward</option>
          </select>
          <button className="dangerBtn" onClick={deleteReviewedProofs} disabled={!reviewedSubmissions.length}>Delete reviewed proofs</button>
        </div>
        {selectedProofIds.length>0&&<div className="proofSelectionNote">{selectedProofIds.length} reviewed proof{selectedProofIds.length===1?"":"s"} selected.</div>}
        <div className="adminSubmissionList">
          {sortedSubmissions.length===0&&<div className="adminEmpty">No submissions found.</div>}
          {sortedSubmissions.map(s=><article className={`adminSubmissionCard ${selectedProofIds.includes(s.id)?"selectedProof":""}`} key={s.id}>
            <div className="submissionIdentity">
              {["approved","rejected"].includes(s.status)&&<input className="proofCheck" type="checkbox" checked={selectedProofIds.includes(s.id)} onChange={()=>toggleProofSelection(s.id)} onClick={e=>e.stopPropagation()} aria-label="Select reviewed proof"/>}
    ...
    {tab==="users"&&<div className="adminContent">
      <div className="adminPanel">
        ...
        <div className="adminUserList">
          ...
          {users.map(u=><article className="adminUserCard clickableUser" key={u.id} onClick={()=>openMemberProfile(u)}>
            ...
            <div className="adminUserActions"><button onClick={e=>{e.stopPropagation();messageUser(u.id)}}>Message</button><button onClick={e=>{e.stopPropagation();act(u.id,u.status==="banned"?"unban":"ban")}}>{u.status==="banned"?"Unban":"Ban"}</button></div>
          </article>)}
        </div>
      </div>
    </div>}
...
{memberProfile&&<div className="memberModal" onClick={()=>setMemberProfile(null)}>
  <div className="memberModalInner" onClick={e=>e.stopPropagation()}>
    <div className="proofModalHeader">
      <div><h3>{memberProfile.user?.first_name||"Telegram member"}</h3><p>@{memberProfile.user?.username||"telegram_user"} · ID {memberProfile.user?.telegram_id}</p></div>
      <button onClick={()=>setMemberProfile(null)}><X/></button>
    </div>
    {memberLoading&&!memberProfile.activity?<div className="adminEmpty">Loading member activity...</div>:<>
      <div className="memberStats">
        <div><b>${Number(memberProfile.user?.balance||0).toFixed(2)}</b><span>Balance</span></div>
        <div><b>{memberProfile.user?.completed_count||0}</b><span>Approved tasks</span></div>
        <div><b>{memberProfile.activity?.length||0}</b><span>Activity events</span></div>
      </div>
      <div className="memberSection">
        <div className="panelTitle"><div><h3>30-day activity</h3><p>Daily events and earnings for this member.</p></div><TrendingUp/></div>
        <div className="memberChart">{Array.from({length:30},(_,i)=>{const d=new Date();d.setDate(d.getDate()-(29-i));const key=d.toISOString().slice(0,10);const x=(memberProfile.chart||[]).find(v=>v.day===key);const n=Number(x?.events||0);const max=Math.max(1,...(memberProfile.chart||[]).map(v=>Number(v.events||0)));return <div className="memberChartCol" key={key}><div className="memberBarTrack"><i style={{height:`${Math.max(n?8:2,(n/max)*100)}%`}} title={`${n} events`}/></div><small>{i%5===0?d.getDate():""}</small></div>})}</div>
      </div>
      <div className="memberSection">
        <div className="panelTitle"><div><h3>Task & activity history</h3><p>Every recorded task action, submission, earning, payout and profile event.</p></div><History/></div>
        <div className="memberActivityList">
          {(memberProfile.activity||[]).map(a=><div className="memberActivityRow" key={`a-${a.id}`}><span>{a.kind.replaceAll("_"," ")}</span><div><b>{a.title||"Account activity"}</b><small>{new Date(a.created_at+"Z").toLocaleString()}</small></div><strong>{Number(a.amount||0)>0?`+$${Number(a.amount).toFixed(2)}`:""}</strong></div>)}
          {(memberProfile.activity||[]).length===0&&<div className="adminEmpty">No activity recorded.</div>}
        </div>
      </div>
      <div className="memberSection">
        <div className="panelTitle"><div><h3>Submission history</h3><p>Review status for every submitted task.</p></div><ListChecks/></div>
        <div className="memberActivityList">
          {(memberProfile.history||[]).map(h=><div className="memberActivityRow" key={`h-${h.id}`}><span className={`historyStatus ${h.status}`}>{h.status}</span><div><b>{h.title||"Deleted task"}</b><small>{new Date(h.created_at+"Z").toLocaleString()}{h.note?` · ${h.note}`:""}</small></div><strong>${Number(h.reward||0).toFixed(2)}</strong></div>)}
          {(memberProfile.history||[]).length===0&&<div className="adminEmpty">No task history.</div>}
        </div>
      </div>
    </>}
  </div>
...
{selectedSubmission&&<div className="proofModal" onClick={()=>setSelectedSubmission(null)}>
      <div className="proofModalInner" onClick={e=>e.stopPropagation()}>
        ...
        <div className="proofGallery">
          {(selectedSubmission.screenshots||[]).map((src,i)=><div className="proofImageCard" key={src}><img src={src} alt={`Proof ${i+1}`}/><span>Proof {i+1} · viewed in app</span></div>)}
          {(!selectedSubmission.screenshots||selectedSubmission.screenshots.length===0)&&<div className="adminEmpty">This reviewed proof has had its screenshots deleted; the review record remains.</div>}
        </div>
