...
function TaskRunner({task,attempt,onDone,setToast}){
  ...
  const [files,setFiles]=useState([]),[note,setNote]=useState(""),[gate,setGate]=useState(!!task.ad_enabled),[adLoading,setAdLoading]=useState(!!task.ad_enabled),[adError,setAdError]=useState("");
  ...
  const runSponsoredAd=async()=>{
    if(adLoading)return false;
    setAdLoading(true);setAdError("");
    try{
      // Do not wait for the API before trying the Monetag SDK. The SDK is the
      // actual sponsor provider; the API only controls whether the gate is enabled.
      const settings=await api("/api/ads").catch(()=>({ads:{enabled:1}}));
      if(settings.ads && Number(settings.ads.enabled)!==1){setGate(false);return true;}
      let show=window.show_11725898;
      for(let i=0;typeof show!=="function"&&i<12;i++){
        await new Promise(r=>setTimeout(r,250));
        show=window.show_11725898;
      }
      if(typeof show!=="function")throw new Error("Sponsor ad is still loading. Please try again.");
      await show({type:"inApp",inAppSettings:{frequency:2,capping:0.1,interval:30,timeout:5,everyPage:false}});
      setGate(false);
      return true;
    }catch(e){
      setAdError(e?.message||"The sponsor ad could not be opened.");
      return false;
    }finally{setAdLoading(false)}
  };
  useEffect(()=>{if(!task.ad_enabled){setGate(false);setAdLoading(false);return;} runSponsoredAd()},[task.id,task.ad_enabled]);
  ...
  return <section>
    ...
    <div className="runner">
      ...
      </>:<div className="adGate"><Sparkles size={32}/><span className="sponsoredBadge">SPONSORED</span><h3>Sponsored activity</h3><p>{adLoading?"Loading the sponsor ad…":"The sponsor ad did not open. You can retry without leaving HillsByte."}</p>{adError&&<small className="adError">{adError}</small>}<button className="primary" disabled={adLoading} onClick={runSponsoredAd}>{adLoading?"Opening sponsor…":"Open sponsored ad"}</button><button className="secondaryBtn" onClick={()=>setGate(false)}>Continue to task</button></div>}
    </div>
  </section>;
}
...
function AdsPage({setToast}){
  const [ads,setAds]=useState(null),[loading,setLoading]=useState(true),[opening,setOpening]=useState(false),[message,setMessage]=useState("");
  ...
  const openSponsor=async()=>{
    ...
    try{
      let show=window.show_11725898;
      for(let i=0;typeof show!=="function"&&i<12;i++){await new Promise(r=>setTimeout(r,250));show=window.show_11725898;}
      if(typeof show!=="function")throw new Error("Sponsor ad is still loading. Please try again.");
      await show({type:"inApp",inAppSettings:{frequency:2,capping:0.1,interval:30,timeout:5,everyPage:false}});
      setMessage("Sponsored activity opened.");
    }catch(e){setMessage(e?.message||"The sponsor ad could not be opened.");}
  ...
  return <section className="adsPage">
    ...
    <div className="sponsorCard"><div className="sponsoredBadge">SPONSORED</div><h3>HillsByte Sponsor</h3><p>Open a sponsored ad directly inside HillsByte. You do not need to leave the app.</p><button className="primary" disabled={opening} onClick={openSponsor}>{opening?"Opening sponsor…":"Open sponsored ad"}</button>{message&&<small className="adMessage">{message}</small>}</div>
    ...
    <div className="profilePanel"><div className="panelTitle"><div><h3>How ads work</h3><p>Sponsored ads open inside the Mini App. Eligible task sponsor gates use the same provider.</p></div><Sparkles/></div><div className="adSteps"><span>01 <b>Open a sponsored activity</b></span><span>02 <b>Complete it normally</b></span><span>03 <b>Return to the task</b></span></div></div>
