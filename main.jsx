...
async function waitForMonetag(timeout=7000){
  const started=Date.now();
  while(typeof window.show_11725898!=="function" && Date.now()-started<timeout){
    await new Promise(r=>setTimeout(r,150));
  }
  return typeof window.show_11725898==="function";
}

async function showMonetagRewarded(setToast){
  if(!(await waitForMonetag())){
    setToast("Sponsor ad is still loading. Please try again in a moment.");
    return false;
  }
  try{
    setToast("Opening sponsor ad…");
    await window.show_11725898();
    await api("/api/ads/complete",{method:"POST",body:JSON.stringify({telegramId:String(tgUser().id)})});
    return true;
  }catch(e){
    setToast("No sponsor ad was available right now. The task was not completed.");
    return false;
  }
}
...
function App(){
  ...
  const [admin,setAdmin]=useState(false),[unread,setUnread]=useState(0),[broadcasts,setBroadcasts]=useState([]),[events,setEvents]=useState([]);
  ...
  const load=async()=>{
    try{
      ...
      setMe(d.user);setTasks(d.tasks||[]);setAdmin(!!d.admin);setBroadcasts(d.broadcasts||[]);setEvents(d.events||[]);
      try{
  ...
  const startTask=async task=>{
    try{
      ...
      if(d.sponsorRequired){
        setSelected(task);
        setTab("task");
        const watched=await showMonetagRewarded(setToast);
        if(!watched){ setSelected(null); setTab("home"); return; }
        await api(`/api/tasks/${task.id}/sponsor-complete`,{method:"POST",body:JSON.stringify({telegramId:String(tgUser().id)})});
        const started=await api(`/api/tasks/${task.id}/start`,{method:"POST",body:JSON.stringify({telegramId:String(tgUser().id)})});
        if(!started.attempt) throw new Error("The task could not be started after the sponsor step.");
        setRunning(started.attempt);
        setToast("Sponsor step completed. The task is now open.");
        return;
      }
      setRunning(d.attempt);setSelected(task);setTab("task");
    }catch(e){setToast(e.message);setRunning(null);setSelected(null)}
  };
...
function TaskRunner({task,attempt,onDone,setToast}){
  ...
  const [files,setFiles]=useState([]),[note,setNote]=useState("");
  ...
  return <section>
    ...
    <div className="runner">
      ...
      <div className="instructions"><h3>How to complete</h3><p>{task.description}</p>{task.url&&<a href={task.url} target="_blank" rel="noreferrer">Open task link ↗</a>}</div>
...
function AdsPage({setToast}){
  const [ads,setAds]=useState(null),[loading,setLoading]=useState(true),[busy,setBusy]=useState(false);
  const load=()=>api(`/api/ads?telegramId=${encodeURIComponent(tgUser().id)}`).then(d=>setAds(d.ads)).catch(e=>setToast(e.message)).finally(()=>setLoading(false));
  useEffect(()=>{load()},[]);
  const watch=async()=>{
    if(busy)return;
    setBusy(true);
    try{
      const ok=await showMonetagRewarded(setToast);
      if(ok){setToast("Sponsor ad completed.");await load();}
    }finally{setBusy(false)}
  };
  const available=!!ads?.enabled && Number(ads?.watched_today||0)<Number(ads?.daily_limit||0);
  return <section className="adsPage">
    <div className="adsHero"><div className="adsIcon"><Megaphone/></div><div><span className="eyebrow">REWARDS CENTER</span><h2>Ads & Sponsors</h2><p>Sponsored ads are separate from the Task Marketplace.</p></div></div>
    <div className="adStatusCard"><div><span className="muted">Sponsor ads</span><b>{loading?"Checking…":ads?.enabled?"Available":"Currently unavailable"}</b></div><span className={`status ${ads?.enabled?"active":"off"}`}>{ads?.enabled?"ON":"OFF"}</span></div>
    <div className="adInfoGrid"><div><Coins/><b>${Number(ads?.reward_amount||0).toFixed(2)}</b><span>Configured reward</span></div><div><Clock3/><b>{Number(ads?.cooldown||0)}s</b><span>Cooldown</span></div><div><ListChecks/><b>{Number(ads?.daily_limit||0)}</b><span>Daily limit</span></div></div>
    <div className="profilePanel">
      <div className="panelTitle"><div><h3>Watch sponsor ad</h3><p>This is separate from the Task Marketplace. Watching it here does not complete a task.</p></div><Sparkles/></div>
      <button className="primary" disabled={!available||busy} onClick={watch}>{busy?"Opening sponsor…":available?"Watch sponsor ad":"Not available right now"}</button>
      <p className="muted adCounter">{Number(ads?.watched_today||0)} watched today · Sponsor ads do not approve proofs.</p>
    </div>
    <div className="profilePanel"><div className="panelTitle"><div><h3>Task sponsor step</h3><p>When a task has its sponsor gate enabled, it appears once before that task starts.</p></div><ListChecks/></div><div className="adSteps"><span>01 <b>Tap a task</b></span><span>02 <b>Watch one sponsor ad</b></span><span>03 <b>Return to the task</b></span><span>04 <b>Complete and submit the task normally</b></span></div></div>
    {!ads?.enabled&&<div className="profileEmpty">The administrator has not enabled sponsor ads yet.</div>}
  </section>;
}
