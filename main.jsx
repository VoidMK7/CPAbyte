...
const proofUrl=(submissionId,index)=>`/api/admin/submissions/${submissionId}/proof/${index}`;
        ...
        <div className="adminSubmissionList">
          ...
          {filteredProofs.map(s=><article className="adminSubmissionCard" key={s.id}>
            ...
            <div className="proofThumbs">
              {(s.screenshots||[]).slice(0,3).map((src,i)=><button key={`${s.id}-${i}`} onClick={()=>{setSelectedSubmission(s);setSelectedProofIndex(i);setProofZoom(false)}}><img src={proofUrl(s.id,i)} alt={`Proof ${i+1}`} onError={e=>{e.currentTarget.style.visibility="hidden"}}/></button>)}
...
{selectedSubmission&&<div className="proofModal" onClick={()=>setSelectedSubmission(null)}>
      <div className="proofModalInner" onClick={e=>e.stopPropagation()}>
        ...
        <div className="proofViewer">
          <div className={`proofFullImage ${proofZoom?"zoomed":""}`}><img src={proofUrl(selectedSubmission.id,selectedProofIndex)} alt={`Proof ${selectedProofIndex+1}`} onError={e=>{e.currentTarget.style.display="none"}}/></div>
