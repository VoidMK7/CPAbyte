...
// Generic sponsor-ad hook. The app itself does not ship an ad-network SDK.
// If an approved ad provider exposes a window-level showSponsorAd function,
// it can be called here without coupling task completion to navigation.
async function showSponsorAd(){
  const fn=window.showSponsorAd;
  if(typeof fn!=="function") return {shown:false,reason:"unavailable"};
  try{
    await fn();
    return {shown:true};
  }catch(e){
    console.warn("Sponsor ad unavailable:",e);
...
function App(){
  ...
  const startTask=async task=>{
    try{
      const first=await api(`/api/tasks/${task.id}/start`,{method:"POST",body:JSON.stringify({telegramId:String(tgUser().id)})});
      if(first.needsAd){
        setToast("Opening sponsor…");
        await showSponsorAd();
        await api(`/api/tasks/${task.id}/ad-view`,{
          method:"POST",
          body:JSON.stringify({telegramId:String(tgUser().id)})
        });
        const after=await api(`/api/tasks/${task.id}/start`,{
          method:"POST",
          body:JSON.stringify({telegramId:String(tgUser().id)})
        });
        if(!after.attempt) throw new Error("Task could not be started.");
        setRunning(after.attempt);setSelected(task);setTab("task");
        return;
      }
      if(!first.attempt) throw new Error("Task could not be started.");
      setRunning(first.attempt);setSelected(task);setTab("task");
    }catch(e){setToast(e.message)}
  };
...
function AdsPage({setToast}){
  const [status,setStatus]=useState(null),[loading,setLoading]=useState(true),[watching,setWatching]=useState(false);

  const load=async()=>{
    try{
      const d=await api(`/api/ads/status?telegramId=${encodeURIComponent(tgUser().id)}`);
      setStatus(d);
    }catch(e){setToast(e.message)}
  ...
  const watch=async()=>{
    ...
    try{
      setWatching(true);
      await api("/api/ads/watch-start",{
        method:"POST",
        body:JSON.stringify({telegramId:String(tgUser().id)})
      });
      const result=await showSponsorAd();
      if(result.shown) setToast("Sponsored ad finished.");
      else setToast("No sponsor ad was available right now.");
      await load();
    }catch(e){setToast(e.message)}
  ...
  const a=status?.ads;
  const canWatch=!!status?.canWatch && !watching;

  return <section className="adsPage">
    <div className="adsHero">
      <div className="adsIcon"><Megaphone/></div>
      <div><span className="eyebrow">SPONSOR CENTER</span><h2>Sponsored Ads</h2><p>Watch an optional sponsor ad from a separate section.</p></div>
    </div>

    <div className="adStatusCard">
      <div><span className="muted">Sponsor service</span><b>{loading?"Checking…":a?.enabled?"Available":"Currently unavailable"}</b></div>
      <span className={`status ${a?.enabled?"active":"off"}`}>{a?.enabled?"ON":"OFF"}</span>
    </div>

    <div className="adInfoGrid">
      <div><Coins/><b>{Number(a?.reward_amount||0)>0?`$${Number(a.reward_amount).toFixed(2)}`:"—"}</b><span>Configured reward</span></div>
      <div><Clock3/><b>{Number(a?.cooldown||0)}s</b><span>Cooldown</span></div>
      <div><ListChecks/><b>{Number(a?.daily_limit||0)}</b><span>Daily limit</span></div>
    </div>

    <div className="profilePanel sponsorWatchPanel">
      <div className="panelTitle"><div><h3>Watch sponsor ad</h3><p>This is separate from the Task Marketplace.</p></div><Sparkles/></div>
      <button className="primary sponsorWatchButton" disabled={!canWatch} onClick={watch}>
        {watching?"Opening sponsor…":canWatch?"Watch sponsored ad":"Not available right now"}
      </button>
      <small className="muted sponsorNote">
        {status?.watchedToday!=null?`${status.watchedToday} watched today · `:""}
        Sponsored ads do not complete tasks or approve proofs.
      </small>
    </div>

    <div className="profilePanel">
      <div className="panelTitle"><div><h3>Task sponsor step</h3><p>When a task has its sponsor gate enabled, it appears once before that task's timer starts.</p></div><ListChecks/></div>
      <div className="adSteps">
        <span>01 <b>Tap a task</b></span>
        <span>02 <b>One sponsor step appears</b></span>
        <span>03 <b>Return to the task</b></span>
        <span>04 <b>Complete the task and submit proof</b></span>
      </div>
    </div>
  </section>;
}
