...
async function restoreDatabase(){
  ...
  try{
    ...
    await ensureStorageBucket(SUPABASE_BUCKET,false);
...
CREATE TABLE IF NOT EXISTS community_posts(
 id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER NOT NULL,message TEXT NOT NULL,created_at TEXT DEFAULT CURRENT_TIMESTAMP
);
...
app.get("/api/bootstrap",async(req,res)=>{
 try{
  ...
  res.json({user:publicUser(u),tasks,taskCount:tasks.length,broadcasts,events,admin:isAdmin(tid),reviewer,moderator,reviewerApplication:application?.status||null,channel:CHANNEL_USERNAME,minWithdrawal:Math.max(1,Number(process.env.MIN_WITHDRAWAL||1))});
...
app.post("/api/withdrawals",(req,res)=>{
 try{
  const u=getUser(req.body.telegramId);requireActive(u);
  const minimum=Math.max(1,Number(process.env.MIN_WITHDRAWAL||1));
  const amount=Number(req.body.amount);
  if(!Number.isFinite(amount)||amount<=0)throw new Error("Enter a valid withdrawal amount.");
  if(amount<minimum)throw new Error(`Minimum withdrawal is $${minimum.toFixed(2)}.`);
  if(amount>Number(u.balance)+0.000001)throw new Error("Withdrawal amount cannot be greater than your available balance.");
  const method=req.body.method==="crypto"?"USDT":"Bank";
  const details={...req.body};delete details.telegramId;delete details.method;delete details.amount;
  db.prepare("INSERT INTO withdrawals(user_id,amount,method,details) VALUES(?,?,?,?)").run(u.id,amount,method,JSON.stringify(details));
  db.prepare("UPDATE users SET balance=balance-?,pending_balance=pending_balance+? WHERE id=?").run(amount,amount,u.id);
  addActivity(u.id,"withdrawal",-amount,null);
  addNotification(u.id,"Withdrawal requested",`Your $${amount.toFixed(2)} withdrawal request is pending review.`,"withdrawal");
  res.json({ok:true,amount,remaining:Number(u.balance)-amount});
 }catch(e){res.status(400).json({error:e.message})}
});
...
app.get("/api/community/posts",(req,res)=>{
 try{
  const posts=db.prepare(`SELECT p.id,p.message,p.created_at,u.first_name,u.username FROM community_posts p JOIN users u ON u.id=p.user_id WHERE u.status='active' ORDER BY p.id DESC LIMIT 100`).all();
  res.json({posts});
 }catch(e){res.status(400).json({error:e.message})}
});
app.post("/api/community/posts",(req,res)=>{
 try{
  ...
  if(message.length>1000) throw new Error("Community messages are limited to 1000 characters.");
  const info=db.prepare("INSERT INTO community_posts(user_id,message) VALUES(?,?)").run(u.id,message);
  addActivity(u.id,"community_post",0,null);
...
app.post("/api/admin/tasks",admin,(req,res)=>{
 try{
  ...
  const info=db.prepare("INSERT INTO tasks(title,short_description,description,url,reward,max_slots,daily_slots,timer_seconds,logo_url,image_url,ad_enabled,platform,action_type,action_label) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)")
   .run(String(b.title).trim(),String(b.short_description||"").trim(),String(b.description).trim(),String(b.url||"").trim(),Number(b.reward||0),max,daily,timer,"","",b.ad_enabled?1:0,String(b.platform||"general"),String(b.action_type||"visit"),String(b.action_label||"Open task"));
...
app.put("/api/admin/tasks/:id",admin,(req,res)=>{
 try{
  ...
  if(!String(b.title||"").trim()||!String(b.description||"").trim()) throw new Error("Task title and description are required.");
  db.prepare(`UPDATE tasks SET title=?,short_description=?,description=?,url=?,reward=?,max_slots=?,daily_slots=?,timer_seconds=?,ad_enabled=?,platform=?,action_type=?,action_label=? WHERE id=?`)
...
app.delete("/api/admin/events/:id",admin,(req,res)=>{db.prepare("UPDATE events SET active=0 WHERE id=?").run(req.params.id);res.json({ok:true})});
...
User message: ${question}`
      ...
      if(r.ok && text)return res.json({answer:text,ai:true});
      } catch {}
    }
