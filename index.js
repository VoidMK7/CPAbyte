...
CREATE TABLE IF NOT EXISTS task_sponsor_completions(
 id INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE TABLE IF NOT EXISTS sponsor_ad_views(
 id INTEGER PRIMARY KEY AUTOINCREMENT,
 user_id INTEGER NOT NULL,
 created_at TEXT DEFAULT CURRENT_TIMESTAMP
);
...
app.get("/api/bootstrap",async(req,res)=>{
 try{
  ...
  res.json({user:publicUser(u),tasks,taskCount:tasks.length,broadcasts,events,admin:isAdmin(tid),channel:CHANNEL_USERNAME});
...
app.post("/api/tasks/:id/start",(req,res)=>{
 try{
  ...
  // A sponsor gate is a separate step. It must be completed once per user/task
  // before the actual task attempt can start. It never opens instead of the task.
  if(Number(t.ad_enabled)===1){
    const sponsor=db.prepare("SELECT id FROM task_sponsor_completions WHERE user_id=? AND task_id=? LIMIT 1").get(u.id,t.id);
    if(!sponsor){
      return res.json({sponsorRequired:true,taskId:t.id});
    }
  }
...
app.post("/api/tasks/:id/sponsor-complete",(req,res)=>{
 try{
  const u=getUser(req.body.telegramId); requireActive(u);
  const t=db.prepare("SELECT id,active,ad_enabled FROM tasks WHERE id=?").get(req.params.id);
  if(!t||!t.active)throw new Error("Task unavailable.");
  if(!t.ad_enabled)return res.json({ok:true,alreadyComplete:true});
  const existing=db.prepare("SELECT id FROM task_sponsor_completions WHERE user_id=? AND task_id=? LIMIT 1").get(u.id,t.id);
  if(existing)return res.json({ok:true,alreadyComplete:true});
  db.prepare("INSERT INTO task_sponsor_completions(user_id,task_id) VALUES(?,?)").run(u.id,t.id);
  res.json({ok:true});
 }catch(e){res.status(400).json({error:e.message})}
});
...
app.get("/api/ads",async(req,res)=>{
  try{
    ...
    const tid=String(req.query.telegramId||"");
    let userId=null;
    if(tid){ const u=getUser(tid); userId=u?.id||null; }
    const today=day();
    const watchedToday=userId?db.prepare("SELECT COUNT(*) AS c FROM sponsor_ad_views WHERE user_id=? AND date(created_at)=?").get(userId,today)?.c||0:0;
    const last=userId?db.prepare("SELECT created_at FROM sponsor_ad_views WHERE user_id=? ORDER BY id DESC LIMIT 1").get(userId):null;
    const nextAvailableAt=last?new Date(new Date(last.created_at.replace(" ","T")+"Z").getTime()+Number(a.cooldown||0)*1000).getTime():0;
    res.json({ads:{...a,watched_today:watchedToday,next_available_at:nextAvailableAt}});
  }catch(e){res.status(400).json({error:e.message})}
});
...
app.post("/api/ads/complete",async(req,res)=>{
  try{
    ...
    if(!a?.enabled)throw new Error("Sponsor ads are not enabled right now.");
    const today=day();
    const count=db.prepare("SELECT COUNT(*) AS c FROM sponsor_ad_views WHERE user_id=? AND date(created_at)=?").get(u.id,today)?.c||0;
    if(count>=Number(a.daily_limit||0))throw new Error("Today's sponsor-ad limit has been reached.");
    const last=db.prepare("SELECT created_at FROM sponsor_ad_views WHERE user_id=? ORDER BY id DESC LIMIT 1").get(u.id);
    if(last){
      const next=new Date(new Date(last.created_at.replace(" ","T")+"Z").getTime()+Number(a.cooldown||0)*1000).getTime();
      if(Date.now()<next)throw new Error(`Please wait ${Math.ceil((next-Date.now())/1000)}s before another sponsor ad.`);
    }
    // This endpoint records the client-side Monetag completion signal. For
    // cash rewards, a Monetag postback should be used when available.
    db.prepare("INSERT INTO sponsor_ad_views(user_id) VALUES(?)").run(u.id);
    res.json({ok:true,watched_today:count+1});
  }catch(e){res.status(400).json({error:e.message})}
});
