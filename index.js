...
async function restoreDatabase(){
  ...
  // Never overwrite a healthy local database with an older cloud snapshot.
  // Render Free starts without the file, so a missing DB is the safe restore case.
...
async function ensureSupabaseBucket(){
  if(!SUPABASE_URL||!SUPABASE_SERVICE_ROLE_KEY) return false;
  try{
    const r=await fetch(`${SUPABASE_URL}/storage/v1/bucket/${encodeURIComponent(SUPABASE_BUCKET)}`,{
  ...
  }catch(e){ console.error("Supabase bucket check:",e.message); return false; }
}
async function uploadFileToSupabase(localPath,key,contentType){
  if(!SUPABASE_URL||!SUPABASE_SERVICE_ROLE_KEY) return null;
  await ensureSupabaseBucket();
  const body=fs.readFileSync(localPath);
  const r=await fetch(`${SUPABASE_URL}/storage/v1/object/${encodeURIComponent(SUPABASE_BUCKET)}/${key.split("/").map(encodeURIComponent).join("/")}`,{
    method:"POST",
    headers:{
      "Authorization":`Bearer ${SUPABASE_SERVICE_ROLE_KEY}`,
      "apikey":SUPABASE_SERVICE_ROLE_KEY,
  ...
  if(!r.ok){
    const text=await r.text().catch(()=>"");
    throw new Error(`Supabase media upload failed (${r.status}) ${text.slice(0,200)}`);
  }
  return `supabase://${key}`;
}
async function signedMediaUrl(value,expires=3600){
  if(!value) return "";
  if(!String(value).startsWith("supabase://")) return value;
  if(!SUPABASE_URL||!SUPABASE_SERVICE_ROLE_KEY) return "";
  const key=String(value).slice("supabase://".length);
  const r=await fetch(`${SUPABASE_URL}/storage/v1/object/sign/${encodeURIComponent(SUPABASE_BUCKET)}/${key.split("/").map(encodeURIComponent).join("/")}`,{
    method:"POST",
    headers:{"Authorization":`Bearer ${SUPABASE_SERVICE_ROLE_KEY}`,"apikey":SUPABASE_SERVICE_ROLE_KEY,"Content-Type":"application/json"},
  ...
  return d.signedURL ? `${SUPABASE_URL}/storage/v1${d.signedURL.startsWith("/")?d.signedURL:`/${d.signedURL}`}` : "";
}
async function deleteStoredMedia(value){
  if(!value) return;
  if(String(value).startsWith("supabase://")){
    if(!SUPABASE_URL||!SUPABASE_SERVICE_ROLE_KEY)return;
    const key=String(value).slice("supabase://".length);
    await fetch(`${SUPABASE_URL}/storage/v1/object/${encodeURIComponent(SUPABASE_BUCKET)}/${key.split("/").map(encodeURIComponent).join("/")}`,{
      method:"DELETE",
      headers:{"Authorization":`Bearer ${SUPABASE_SERVICE_ROLE_KEY}`,"apikey":SUPABASE_SERVICE_ROLE_KEY}
    }).catch(()=>{});
    return;
  }
  const local=path.join(UPLOAD_DIR,path.basename(String(value)));
  if(fs.existsSync(local)) fs.unlinkSync(local);
}
async function persistProofFiles(files,userId,submissionKey){
  const paths=[];
  for(const f of files){
    if(SUPABASE_URL&&SUPABASE_SERVICE_ROLE_KEY){
      const key=`proofs/${userId}/${submissionKey}/${path.basename(f.path)}`;
      paths.push(await uploadFileToSupabase(f.path,key,f.mimetype));
      try{fs.unlinkSync(f.path)}catch{}
    }else{
      paths.push(`/uploads/${path.basename(f.path)}`);
    }
  }
  return paths;
}
async function migrateLocalProofs(){
  if(!SUPABASE_URL||!SUPABASE_SERVICE_ROLE_KEY) return;
  try{
    await ensureSupabaseBucket();
    const rows=db.prepare("SELECT id,user_id,screenshots FROM submissions WHERE screenshots IS NOT NULL AND screenshots<>''").all();
    for(const row of rows){
      const current=parseScreenshots(row.screenshots);
      let changed=false;
      const next=[];
      for(const value of current){
        if(String(value).startsWith("supabase://")){ next.push(value); continue; }
        const local=path.join(UPLOAD_DIR,path.basename(String(value)));
        if(fs.existsSync(local)){
          try{
            const stored=await uploadFileToSupabase(local,`proofs/${row.user_id}/${row.id}/${path.basename(local)}`,({".png":"image/png",".webp":"image/webp",".jpg":"image/jpeg",".jpeg":"image/jpeg"}[path.extname(local).toLowerCase()]||"image/jpeg"));
            next.push(stored); changed=true;
            try{fs.unlinkSync(local)}catch{}
          }catch(e){ console.error(`Proof migration ${row.id}:`,e.message); next.push(value); }
        }else next.push(value);
      }
      if(changed) db.prepare("UPDATE submissions SET screenshots=? WHERE id=?").run(JSON.stringify(next),row.id);
    }
    await syncDatabase();
  }catch(e){console.error("Proof migration skipped:",e.message)}
}
...
app.post("/api/tasks/:id/start",(req,res)=>{
 try{
  ...
  addActivity(u.id,"task_start",0,t.id);
...
app.post("/api/submissions",upload.array("screenshots",3),async(req,res)=>{
 try{
  ...
  const submissionId=db.prepare("SELECT COALESCE(MAX(id),0)+1 AS id FROM submissions").get().id;
  const shots=await persistProofFiles(files,u.id,submissionId);
...
app.post("/api/withdrawals",(req,res)=>{
 try{
  ...
  addActivity(u.id,"withdrawal",u.balance,null);
  res.json({ok:true});
 }catch(e){res.status(400).json({error:e.message})}
});
...
app.post("/api/profile",(req,res)=>{
 try{
  ...
  addActivity(u.id,"profile_update",0,null);
...
app.post("/api/promo/redeem",(req,res)=>{
 try{
  ...
  addActivity(u.id,"promo",p.amount,null);
...
app.get("/api/admin/users/:id",admin,async(req,res)=>{
  try{
    ...
    const history=db.prepare(`SELECT s.id,s.status,s.created_at,s.note,t.id task_id,t.title,t.reward
      FROM submissions s LEFT JOIN tasks t ON t.id=s.task_id WHERE s.user_id=? ORDER BY s.id DESC LIMIT 200`).all(u.id);
    const activity=db.prepare(`SELECT a.id,a.kind,a.amount,a.created_at,a.task_id,t.title
      FROM activity a LEFT JOIN tasks t ON t.id=a.task_id WHERE a.user_id=? ORDER BY a.id DESC LIMIT 300`).all(u.id);
    const chart=db.prepare(`SELECT substr(created_at,1,10) day,COUNT(*) events,COALESCE(SUM(CASE WHEN kind='earning' THEN amount ELSE 0 END),0) earnings
      FROM activity WHERE user_id=? AND created_at>=datetime('now','-29 days') GROUP BY substr(created_at,1,10) ORDER BY day`).all(u.id);
    const notifications=db.prepare("SELECT * FROM notifications WHERE user_id=? ORDER BY id DESC LIMIT 100").all(u.id);
    res.json({user:publicUser(u),history,activity,chart,notifications});
  }catch(e){res.status(400).json({error:e.message})}
});
...
app.post("/api/admin/submissions/:id",admin,(req,res)=>{
 ...
 if(status==="approved"){
  ...
  const tx=db.transaction(()=>{
   ...
   addActivity(u.id,"proof_approved",0,t.id);
  ...
  addActivity(s.user_id,"proof_rejected",0,s.task_id);
...
app.get("/api/admin/submissions",admin,async(req,res)=>{
  ...
  const submissions=await Promise.all(rows.map(async s=>({
    ...s,
    screenshots:(await Promise.all(parseScreenshots(s.screenshots).map(x=>signedMediaUrl(x)))).filter(Boolean),
    proof_count:parseScreenshots(s.screenshots).length
  })));
  res.json({submissions});
});

app.delete("/api/admin/submissions/reviewed-proofs",admin,async(req,res)=>{
  try{
    const requested=Array.isArray(req.body?.ids)?req.body.ids.map(Number).filter(Number.isFinite):[];
    const rows=requested.length
      ? db.prepare(`SELECT id,screenshots FROM submissions WHERE id IN (${requested.map(()=>"?").join(",")}) AND status IN ('approved','rejected')`).all(...requested)
      : db.prepare("SELECT id,screenshots FROM submissions WHERE status IN ('approved','rejected') AND screenshots IS NOT NULL AND screenshots<>''").all();
    for(const row of rows){
      for(const src of parseScreenshots(row.screenshots)) await deleteStoredMedia(src);
      db.prepare("UPDATE submissions SET screenshots='[]' WHERE id=?").run(row.id);
    }
