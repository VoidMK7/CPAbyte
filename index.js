...
db.exec(`
CREATE TABLE IF NOT EXISTS task_ad_views(
...
app.post("/api/tasks/:id/start",(req,res)=>{
 try{
  ...
  // Sponsor gate: each member sees the sponsor step at most once per task.
  // The task timer is NOT started until the sponsor step has been handled.
  if(Number(t.ad_enabled)===1){
    const seen=db.prepare("SELECT 1 FROM task_ad_views WHERE user_id=? AND task_id=?").get(u.id,t.id);
    if(!seen){
      return res.json({needsAd:true,taskId:t.id});
    }
  }
  ...
  if(existing){
    ...
    return res.json({needsAd:false,attempt:{id:existing.id,expires_at:expires}});
  }
  ...
  res.json({needsAd:false,attempt:{id:info.lastInsertRowid,expires_at:expires}});
 }catch(e){res.status(400).json({error:e.message})}
});

app.post("/api/tasks/:id/ad-view",(req,res)=>{
 try{
  const u=getUser(req.body.telegramId); requireActive(u);
  ...
  if(Number(t.ad_enabled)!==1)return res.json({ok:true,already:true});
  db.prepare("INSERT OR IGNORE INTO task_ad_views(user_id,task_id) VALUES(?,?)").run(u.id,t.id);
  res.json({ok:true});
 }catch(e){res.status(400).json({error:e.message})}
});
...
app.get("/api/ads/status",async(req,res)=>{
 try{
  ...
  const last=db.prepare("SELECT created_at FROM ad_watch_history WHERE user_id=? ORDER BY id DESC LIMIT 1").get(u.id);
  const daily=db.prepare("SELECT COUNT(*) count FROM ad_watch_history WHERE user_id=? AND created_at>=datetime('now','start of day')").get(u.id);
  res.json({
    ads:a||{enabled:0},
    watchedToday:Number(daily?.count||0),
    lastWatchedAt:last?.created_at||null,
    canWatch:!!(a?.enabled) && Number(daily?.count||0)<Number(a?.daily_limit||0) &&
      (!last?.created_at || (Date.now()-new Date(String(last.created_at).replace(" ","T")+"Z").getTime()) >= Number(a?.cooldown||0)*1000)
  });
 }catch(e){res.status(400).json({error:e.message})}
});

app.post("/api/ads/watch-start",(req,res)=>{
 try{
  const u=getUser(req.body.telegramId); requireActive(u);
  const a=db.prepare("SELECT * FROM ad_settings WHERE id=1").get();
  if(!a?.enabled)throw new Error("Sponsored ads are currently unavailable.");
  const daily=db.prepare("SELECT COUNT(*) count FROM ad_watch_history WHERE user_id=? AND created_at>=datetime('now','start of day')").get(u.id);
  if(Number(daily?.count||0)>=Number(a.daily_limit||0))throw new Error("Today's sponsored-ad limit has been reached.");
  const last=db.prepare("SELECT created_at FROM ad_watch_history WHERE user_id=? ORDER BY id DESC LIMIT 1").get(u.id);
  if(last?.created_at){
    const lastMs=new Date(String(last.created_at).replace(" ","T")+"Z").getTime();
    if(Date.now()-lastMs < Number(a.cooldown||0)*1000)throw new Error(`Please wait ${Math.ceil((Number(a.cooldown||0)*1000-(Date.now()-lastMs))/1000)}s before watching another sponsor ad.`);
  }
  db.prepare("INSERT INTO ad_watch_history(user_id) VALUES(?)").run(u.id);
  res.json({ok:true});
 }catch(e){res.status(400).json({error:e.message})}
});
...
app.get("/health",(req,res)=>res.status(200).json({ok:true,service:"HillsByte"}));
...
app.listen(PORT,()=>{
  ...
  // Do not block Render's health check on external services.
  setTimeout(()=>syncDatabase().catch(e=>console.error("Initial backup failed:",e.message)),0);
  if(BOT_TOKEN){
    setTimeout(async()=>{
      try{
